...
export type CreateShaderPRResult = {
  prUrl: string
  prNumber: number
}
...
export async function createShaderPR(
  data: CreateShaderPRInput,
  ghOpts: { token: string; owner: string; repo: string },
  fetchFn: typeof fetch = fetch,
): Promise<CreateShaderPRResult> {
  const opts: GitHubApiOptions = ghOpts
  const shaderName = data.name
  const branchName = `shader/${shaderName}`

  // Check if branch already exists (duplicate submission)
  if (await branchExists(opts, branchName, fetchFn)) {
    throw new Error(
  ...
  const baseCommitSha = await getLatestCommitSha(opts, 'master', fetchFn)
  const commitData = (await githubApi(opts, `/git/commits/${baseCommitSha}`, 'GET', undefined, fetchFn)) as {
    tree: { sha: string }
  }
  ...
  // vertex.glsl
  const vertexBlob = await createBlob(opts, data.vertexSource, 'utf-8', fetchFn)
  treeEntries.push({
  ...
  // fragment.glsl
  const fragmentBlob = await createBlob(opts, data.fragmentSource, 'utf-8', fetchFn)
  treeEntries.push({
  ...
  for (const [, recipe] of Object.entries(data.recipes)) {
    const recipeBlob = await createBlob(opts, recipe.code, 'utf-8', fetchFn)
    treeEntries.push({
  ...
  if (data.previewSvg) {
    const previewBlob = await createBlob(opts, data.previewSvg, 'utf-8', fetchFn)
    treeEntries.push({
  ...
  const prBody = [
    ...
    '',
    '---',
    '*Submitted via the ShaderBase CLI/MCP.*',
  ].join('\n')
  ...
  const pr = (await githubApi(opts, '/pulls', 'POST', {
    ...
    base: 'master',
    body: prBody,
  }, fetchFn)) as { html_url: string; number: number }
